...
The Knowledge Base module manages documentation sources and team knowledge. It builds indices describing each source and provides helpers so the AI response module can select and load only the most relevant content for a given query.
...
- Provide retrieval helpers to the AI response module:
  - Provide the full index text for the AI response module to send to the LLM for source selection
...
The index is intended to be small and fast to read at runtime. It MUST be a **UTF-8 text file**. The AI response module may send the full index text to the LLM for source selection, so the format should prioritize readability and stable diffs.
...
The AI response module should not know about filesystem scanning or HTTP caching details. It should call a small retrieval API.
...
At runtime, the AI response module:
...
- The AI response module cites sources using the identifier line from the index.
...
- If no selected sources can be loaded:
  - Return empty source content; the AI response module decides whether to reply.
